<template>
  <div class="border-b-2 border-slate-200">
    <div class="container flex items-center justify-between">
      <div class="w-full">
        <ul class="flex items-center justify-between w-full relative z-40">
          <template v-for="item in dataMenu">
            <li
              :class="item.submenu.length ? 'parent' : ''"
              v-if="!item.parent_id"
            ...
            >
              ...
              />
              <div
                class="menulevel2 bg-white children hidden absolute left-0 top-full py-4 z-40 w-full shadow-md rounded-b-md shadow-slate-200 px-6"
                v-if="item.submenu.length"
              >
